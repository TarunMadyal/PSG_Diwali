...
import { calculateCartPricing, type AppliedCombo, type ComboNudge } from "@/lib/pricing";
...
export function AppProviders({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    const timer = window.setTimeout(() => {
      ...
      try {
        setCart(JSON.parse(localStorage.getItem("psg-cart") ?? "[]") as CartLine[]);
      } catch {
  ...
  const value = useMemo<Value>(
    () => ({
      ...
      setLanguage: (next) => {
        ...
        localStorage.setItem("psg-language", next);
        document.documentElement.lang = next;
      },
      cart,
      addLine: (line) =>
        setCart((items) => {
          const old = items.find((item) => item.lineId === line.lineId);
          return old
            ? items.map((item) =>
      ...
      updateQuantity: (id, quantity) =>
        setCart((items) =>
          quantity < 1
            ? items.filter((item) => item.lineId !== id)
            : items.map((item) => (item.lineId === id ? { ...item, quantity } : item)),
        ),
      clearCart: () => setCart([]),
...
export function useApp() {
  ...
  if (!value) throw new Error("Missing AppProviders");
  return value;
}
